...
// Мьютекс и состояние для сада
pthread_mutex_t gardenMutex;
pthread_cond_t gardenerCondition;
...
void* gardenerWork(void* params) {
    Gardener* gardener = static_cast<Gardener*>(params);  // Из аргументов делаем садовода
    ...
    while (true) {
        ...
        // Захватываем мьютекс перед проверкой готовности
        pthread_mutex_lock(&gardenMutex);
        while (!ready) {
            pthread_cond_wait(&gardenerCondition, &gardenMutex);
        }
        ready = false;

        // Проверка, свободен ли квадрат для обработки
        if (garden[pos.first][pos.second] == EMPTY) {
            garden[pos.first][pos.second] = trace;
        }
        ...
        // Меняем флаг готовности и будим другого садовника
        ready = true;
        pthread_cond_signal(&gardenerCondition);
        // Освобождаем мьютекс после выполнения работы
        pthread_mutex_unlock(&gardenMutex);
        ...
        if (pos.first == -1 || pos.first == garden.size() || pos.second == -1 || pos.second == garden[0].size()) {
            pos.first -= step.first;
            pos.second -= step.second;
...
void twoGardenerFiller() {
    // Даю сид для рандома (чтобы рандом был хоть какого-то непостоянного качества)
    srand(time(nullptr));
    ...
    // Создание потоков, и мьютекса, и состояния
    pthread_t thread1, thread2;
    pthread_mutex_init(&gardenMutex, nullptr);
    pthread_cond_init(&gardenerCondition, nullptr);
    ...
    // Заканичиваю полностью работу потоков
